...
exports.userCreate= async (req,res)=>{
    ...
    try {
        ...
        const data= await UserSchema.create(
            {
                pic,username,email,password: hashPassword 
            }
        )
...
exports.userUpdate= async (req,res)=>{
    try {
        const data= await UserSchema.findByIdAndUpdate(req.body.id,req.body,{new:true})
        res.status(200).json(data)
        console.log(data);
...
exports.login= async (req,res)=>{
    try {
        const data= await UserSchema.findOne({email:req.body.email}).select("+password")
         if(!data)
         {
...
exports.addCart= async (req,res)=>{
    const cart={
        pic:req.body.pic,name:req.body.name,price:req.body.price,quantity:req.body.quantity
    }
    try {
        const data= await UserSchema.findById(req.userAllData.id)
        // console.log(req.userAllData.id)
...
exports.removeCart= async (req,res)=>{
    try {
        ...
        data.cart.filter((i)=>i._id==req.params.id && data.cart.pull({_id: req.params.id}) )
        // data.cart.push(cart)
